...
# Nouveau : get_excel_backup() — backup .xlsx 3 onglets (securite si PDF plante)
# =============================================================================
...
# ---------------------------------------------------------------------------
# FILTRAGE FONDS — clause IN() dynamique, zero injection SQL
# ---------------------------------------------------------------------------

def _fonds_clause(fonds_filter, alias="p"):
    """
    Retourne (clause_sql, params_list).
    None ou vide -> pas de filtrage -> ("", [])
    Liste non vide -> ("AND alias.fonds IN (?,?,...)", [...])
    """
    if not fonds_filter:
        return "", []
    placeholders = ",".join(["?"] * len(fonds_filter))
    clause = "AND {}.fonds IN ({})".format(alias, placeholders)
    return clause, list(fonds_filter)


# ---------------------------------------------------------------------------
# NETTOYAGE TYPES — contrat strict sur tous les DataFrames pipeline
# ---------------------------------------------------------------------------
...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS pipeline (
        ...
        fonds              TEXT NOT NULL
            CHECK(fonds IN ('Global Value','International Fund','Income Builder',
...
def _insert_demo_data(conn):
    ...
    pipeline = [
        (1,  "Global Value",       "Funded",        50_000_000,  45_000_000, 42_000_000,
         None, None, (today + timedelta(days=15)).isoformat(), "Sophie Laurent"),
        (2,  "Income Builder",     "Soft Commit",   30_000_000,  28_000_000, 0,
         None, None, (today - timedelta(days=3)).isoformat(),  "Marc Dupont"),
        (3,  "Private Debt",       "Due Diligence", 100_000_000, 100_000_000, 0,
         None, None, (today + timedelta(days=7)).isoformat(),  "Sophie Laurent"),
        (4,  "Resilient Equity",   "Funded",        75_000_000,  80_000_000, 78_000_000,
         None, None, (today + timedelta(days=30)).isoformat(), "Karim Belhadj"),
        (5,  "International Fund", "Initial Pitch", 20_000_000,  20_000_000, 0,
         None, None, (today - timedelta(days=10)).isoformat(), "Marc Dupont"),
        (6,  "Active ETFs",        "Lost",          40_000_000,  35_000_000, 0,
         "Pricing", "Vanguard", (today + timedelta(days=60)).isoformat(), "Karim Belhadj"),
        (7,  "Global Value",       "Funded",        120_000_000, 115_000_000, 110_000_000,
         None, None, (today + timedelta(days=20)).isoformat(), "Sophie Laurent"),
        (8,  "Private Debt",       "Soft Commit",   200_000_000, 180_000_000, 0,
         None, None, (today - timedelta(days=1)).isoformat(),  "Karim Belhadj"),
        (9,  "Income Builder",     "Paused",        15_000_000,  12_000_000, 0,
         "Macro", "Internal", (today + timedelta(days=90)).isoformat(), "Marc Dupont"),
        (10, "Resilient Equity",   "Due Diligence", 60_000_000,  60_000_000, 0,
         None, None, (today + timedelta(days=5)).isoformat(),  "Sophie Laurent"),
    ]
    c.executemany(
    ...
    activites = [
        ...
        (7,  (today - timedelta(days=3)).isoformat(),  "Review trimestrielle", "Meeting"),
        (8,  (today - timedelta(days=1)).isoformat(),  "Negociation conditions LP", "Call"),
        (5,  (today - timedelta(days=6)).isoformat(),  "Presentation initiale", "Meeting"),
...
def get_all_clients():
    ...
    df = pd.read_sql_query(
        "SELECT id, nom_client, type_client, region FROM clients ORDER BY nom_client", conn
    )
    conn.close()
    return df
...
# ---------------------------------------------------------------------------
# LECTURES — DRILL-DOWN pour modales st.dialog
# ---------------------------------------------------------------------------
...
def get_funded_deals_detail(fonds_filter=None):
    ...
    conn = get_connection()
    query = (
        "SELECT c.nom_client AS Client, p.fonds AS Fonds,"
        " c.type_client AS Type, c.region AS Region,"
        " p.funded_aum AS AUM_Finance, p.sales_owner AS Commercial"
        " FROM pipeline p JOIN clients c ON c.id = p.client_id"
...
def get_active_deals_detail(fonds_filter=None):
    ...
    query = (
        "SELECT c.nom_client AS Client, p.fonds AS Fonds, p.statut AS Statut,"
        " p.revised_aum AS AUM_Revise, p.sales_owner AS Commercial,"
    ...
    df = pd.read_sql_query(query, conn, params=fonds_params if fonds_params else None)
    conn.close()
    if "AUM_Revise" in df.columns:
        df["AUM_Revise"] = pd.to_numeric(df["AUM_Revise"], errors="coerce").fillna(0.0)
    return df
...
def upsert_pipeline_from_df(df):
    ...
    for _, row in df.iterrows():
        ...
        next_act   = str(row.get("next_action_date", "") or "").strip() or None
        ...
        if existing:
            c.execute(
                ...
                (statut, t_aum, r_aum, f_aum, raison, concurrent, next_act, sales, existing["id"])
            )
            updated += 1
        else:
            c.execute(
                ...
                " VALUES (?,?,?,?,?,?,?,?,?,?)",
                (client_id, fonds, statut, t_aum, r_aum, f_aum, raison, concurrent, next_act, sales)
            )
            inserted += 1
    conn.commit()
    conn.close()
...
# ---------------------------------------------------------------------------
# BACKUP EXCEL — 3 onglets (securite si PDF ou web plante)
# ---------------------------------------------------------------------------

def get_excel_backup(fonds_filter=None):
    """
    Genere un .xlsx avec 3 onglets :
      Pipeline_Actif      — tous les deals (filtre fonds optionnel), AUM en M EUR
      Historique_Audit    — 200 derniers evenements audit
      Referentiel_Clients — tous les clients

    Retourne les bytes du fichier .xlsx.
    Utilise pandas.ExcelWriter + openpyxl.
    """
    buf = io.BytesIO()

    # Onglet 1 — Pipeline
    df_p = get_pipeline_with_clients(fonds_filter).copy()
    for col in ["target_aum_initial", "revised_aum", "funded_aum"]:
        if col in df_p.columns:
            df_p[col] = (df_p[col] / 1_000_000).round(2)
    rename_p = {
        "id": "ID", "nom_client": "Client", "type_client": "Type",
        "region": "Region", "fonds": "Fonds", "statut": "Statut",
        "target_aum_initial": "AUM_Cible_M_EUR",
        "revised_aum":        "AUM_Revise_M_EUR",
        "funded_aum":         "AUM_Finance_M_EUR",
    ...
    df_a = pd.read_sql_query(
        ...
        " FROM audit_log a"
        " JOIN pipeline p ON p.id = a.pipeline_id"
